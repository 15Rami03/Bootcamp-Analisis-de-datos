...
SELECT t.id, t.amount, t.timestamp, c.company_name, c.country
FROM transaction t 
JOIN company c
ON c.id = t.company_id
WHERE c.country IN (
...
SELECT 
    ...
    (SELECT COUNT(*)
     FROM transaction t
     WHERE t.company_id = c.id) AS total_transactions,
    CASE
        WHEN (SELECT COUNT(*)
              FROM transaction t
              WHERE t.company_id = c.id) > 400 THEN 'More than 400'
        ELSE '400 or less'
